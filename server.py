...
<a href="/quieromiweb" class="web-promo" data-reveal aria-label="¿Necesitas una página web para tu negocio? Te la creamos, simple y profesional">
  <span class="web-promo-icon"><span class="web-promo-icon-flip">
    <svg class="icon-face" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="9"/><path d="M3 12h18M12 3c2.5 2.6 4 6 4 9s-1.5 6.4-4 9c-2.5-2.6-4-6-4-9s1.5-6.4 4-9Z"/></svg>
    <svg class="icon-face icon-face-back" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M5 13l4 4L19 7"/></svg>
  </span></span>
...
<section class="section" data-reveal>
  ...
  <p class="hint" style="margin-top:22px">Los ${money(WEB_PRECIO_CREACION)} corresponden solo al diseño de tu sitio. Para publicarlo en un
    dominio real y que quede disponible en internet, necesitas además la suscripción mensual de
    ${money(WEB_PRECIO_SOPORTE)}, que incluye el hosting y todo lo necesario para mantener tu sitio en línea. Así
    tendrás tu web funcionando y disponible para tus clientes de forma permanente.</p>
</section>
...
def detalle(handler, aviso_id, query="", contabilizar=True):
    ...
    if aviso.get("es_demo"):
        titulo_demo = "¡Publícate hoy y empieza a vender más!"
        body_demo = f"""
<section class="need-cta" data-reveal>
  ...
  <form method="post" action="/publicar" class="form" enctype="multipart/form-data">
    ...
    <label>Foto de tu negocio o trabajo (opcional)
      ...
      <span class="hint">JPG, PNG o WebP, máximo 5 MB.
        Idealmente que sea una foto de buena calidad, <strong>sin texto ni bordes</strong> — el texto va mejor en la descripción.</span>
    </label>
